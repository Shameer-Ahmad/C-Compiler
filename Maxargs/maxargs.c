...
maxargs(A_stm stm) {
  ...
  switch (stm->kind) {
    ...
    case A_printStm:
      ...
      return max(count_exp(stm->u.print.exps), maxargs_expList(stm->u.print.exps));

    default:
      assert(!"Error in maxargs()!");
...
maxargs_expList(A_expList expList) {
  ...
  switch (expList->kind) {
    ...
    default:
      ...
      assert(!"Error in maxargs_expList()!");
