...
// typedef struct { int line; int column; } A_Pos;
// typedef int A_Pos;
typedef E_Pos A_Pos;
...
struct A_TypeDec_ {S_Symbol name; A_Type ty;};
...
A_Exp make_A_RecordExp(A_Pos pos, S_Symbol typ, A_EFieldList fields);
...
A_Exp make_A_ArrayExp(A_Pos pos, S_Symbol typ, A_Exp size, A_Exp init);
...
A_Dec make_A_VarDec(A_Pos pos, S_Symbol var, S_Symbol typ, A_Exp init);
...
A_Field make_A_Field(A_Pos pos, S_Symbol name, S_Symbol typ);
...
A_TypeDec make_A_TypeDec(S_Symbol name, A_Type ty);
